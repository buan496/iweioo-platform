...
test("portal and account consume the shared server-side OIDC BFF", () => {
  ...
  const sessionStore = read("packages/auth-bff/src/store-core.ts");
  ...
  const accountSessions = read("apps/account/app/api/auth/sessions/route.ts");
  const accountLogoutAll = read("apps/account/app/auth/logout-all/route.ts");
  ...
  assert.match(authConfig, /AUTH_MAX_SESSIONS_PER_USER/);
  ...
  assert.match(handlers, /handleSessionListGet/);
  assert.match(handlers, /handleSessionRevokeDelete/);
  assert.match(handlers, /handleLogoutAllPost/);
  const safeMetadata = sessionStore.match(/type SessionMetadata = \{([\s\S]*?)\};/)?.[1];
  assert.ok(safeMetadata);
  assert.ok(!/recordKey|refreshLockKey|userKey|Token|handle|userAgent|ipAddress/i.test(safeMetadata));
  assert.match(sessionStore, /type SessionLocator/);
  assert.match(sessionStore, /SESSION_LOCATOR_PREFIX/);
  assert.match(sessionStore, /maxSessionsPerUser/);
  assert.match(portalCallback, /handleAuthorizationCallback/);
  assert.match(accountCallback, /handleAuthorizationCallback/);
  assert.match(accountSessions, /handleSessionListGet/);
  assert.match(accountLogoutAll, /handleLogoutAllPost/);
